...
local creds = window:Tab("公告",'7733993211')
    ...
    local bin = creds:section("信息",true)
    bin:Label("QQ交流群:938564903")
    bin:Label("开发作者QQ号:3673262204")
    bin:Label("开发脚本属于：免费")
    bin:Label("免费的免费的禁止圈狗")
    bin:Label("作者要饿死了，马上去支援")
    bin:Label("世界这么大，我想去看看")
    
local creds = window:Tab("功能类", "6035145364")
    local credits = creds:section("游戏内存", true)
...
credits:Toggle("人物夜视", "", false, function(state)
        if state then
...
local Wredits = creds:section("内存娱乐", true)
...
local Wredits = creds:section("聊天内存", true)
...
local Wredits = creds:section("内存物品", true)
...
local creds = window:Tab("绘制类", "6035145364")
    local credits = creds:section("游戏安全", true)
